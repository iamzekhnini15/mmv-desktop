...
public static class DbInitializer
{
    ...
    private static List<Order> CreateOrders(List<Customer> customers, List<User> users, List<Product> products)
    {
        ...
        var statuses = new[] { OrderStatus.New, OrderStatus.ToFabricate, OrderStatus.InProgress, OrderStatus.QualityCheck, OrderStatus.Ready, OrderStatus.Delivered };
        
        var frames = products.Where(p => p.Category == ProductCategoryEnum.MONTURE).ToList();
        var glasses = products.Where(p => p.Category == ProductCategoryEnum.VERRE).ToList();
        var accessories = products.Where(p => p.Category == ProductCategoryEnum.PLASTIC).ToList();

        // Créer 40 commandes
        for (int i = 0; i < 40; i++)
        {
            var customer = customers[_random.Next(customers.Count)];
            var staff = users.Where(u => u.Role == UserRole.Optician).OrderBy(x => _random.Next()).FirstOrDefault() ?? users[0];
            var orderDate = DateTime.UtcNow.AddDays(-_random.Next(1, 90));

            var order = new Order
            {
                OrderNumber = $"CMD-{DateTime.UtcNow.Year}-{(i + 1):D4}",
                CustomerId = customer.CustomerId,
                StaffId = staff.UserId,
                OrderDate = orderDate,
                EstimatedDelivery = orderDate.AddDays(_random.Next(7, 21)),
                Status = statuses[_random.Next(statuses.Length)],
                Notes = _random.Next(100) < 20 ? "Urgente" : ""
            };

            var orderItems = new List<OrderItem>();

            // Ajouter une monture
            var frame = frames[_random.Next(frames.Count)];
            orderItems.Add(new OrderItem
            {
                ...
                Quantity = 1,
                UnitPrice = frame.SalePrice
            });

            // Ajouter des verres OD et OG
            var glass = glasses[_random.Next(glasses.Count)];
            var hasPrescription = _random.Next(100) < 80;
            ...
            if (hasPrescription)
            {
                ...
                orderItems.Add(new OrderItem
                {
                    ...
                    Quantity = 1,
                    UnitPrice = glass.SalePrice,
                    UsageType = (LensUsageType)_random.Next(0, 3),
                    Sphere = RandomSphere(),
                    Cylinder = _random.Next(100) < 40 ? RandomCylinder() : null,
                    Axis = _random.Next(100) < 40 ? _random.Next(0, 181) : null,
                    Addition = _random.Next(100) < 30 ? RandomAddition() : null
                });
            ...
            // Ajouter parfois un accessoire
            if (_random.Next(100) < 40)
            {
                var accessory = accessories[_random.Next(accessories.Count)];
                orderItems.Add(new OrderItem
                {
                    Order = order,
                    ProductId = accessory.ProductId,
